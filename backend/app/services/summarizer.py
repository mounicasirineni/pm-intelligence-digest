...
- SOURCE FIDELITY RULE: Every bullet must be traceable to a specific claim, data point, or quote in the content body above. You may reason one logical step beyond the source (e.g. identifying an implication), but you may not:
  ...
  UNIVERSALITY TEST: Before writing any bullet that uses universalizing language ('all companies,' 'every PM,' 'the industry is shifting,' 'frontier AI companies are moving toward'), count the number of examples the content body actually provides. If the content describes a single company's move, you may not generalize to 'all companies' or 'the industry.' Scope the bullet to the specific company or example: 'This company's move suggests...' or 'This case shows that...' A single data point is evidence for a pattern, not proof of one. Write the specific example; let the synthesizer identify the cross-source pattern. If the content body provides 3+ examples independently verifying a pattern, you may characterize it as a trend — but only if those examples are in the content body, not in your training knowledge.
  VERIFIED MOTIVATION RULE: Do not assert a company's strategic motivation unless the source explicitly states it. Press releases, company blogs, and first-party announcements almost never state strategic motivations directly — they describe what was done, not why. If the source is a company blog or press release, treat all motivation language as unconfirmed. Do not write 'the company did X likely because Y' or 'this reflects a strategic bet on Z' unless the source text uses those exact words or equivalent language. If the motivation is unclear, write the observable action and its observable consequence: 'the company did X; if sustained, this would...' — not 'the company did X to achieve Y.'
...
- COMPANY MATURITY RULE: If this article's primary subject is a named company, assess whether that company is early-stage or established. Set "company_maturity" to:
    startup     = primary subject is an early-stage or emerging company that has not yet achieved dominant market position AND is valued below $1B. Examples: a pre-Series B startup, a new entrant with no significant market share.
    established = primary subject is (a) a publicly traded company, (b) a subsidiary of a publicly traded company, OR (c) a privately held company with $1B+ valuation AND significant market presence or name recognition. This explicitly includes: Anthropic, OpenAI, Stripe, SpaceX, Databricks, Canva, and other well-funded private companies that are not early-stage startups. If a company has raised $1B+ or is widely reported to have a $1B+ valuation, classify as established regardless of public/private status.
  ...
  MATURITY EDGE CASE RULE: When in doubt between startup and established, ask: is this company a household name in the tech industry? If yes, classify as established. 'Startup' is reserved for companies that a typical tech-industry observer would not already know.
